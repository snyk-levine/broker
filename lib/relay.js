...
module.exports = {
  ...
  setCustomResponseHandler,
};

// Set this if requests/responses to targets
// should be handled by an external client
let customResponseHandler = null;

function setCustomResponseHandler(f) {
  ...
  return (brokerToken) => ({ url, headers = {}, method, body = null, streamingID = '' } = {}, emit) => {
    ...
    filters({ url, method, body, headers }, (error, result) => {
      ...
      if (customResponseHandler) {
        customResponseHandler(req, emit)
        return
      }
